...
data_json <- cbind(data_categorical,data_poisson,data_gaussian,data_ordinal)

colnames(data_json) <- c("categorical1","categorical2","categorical3","poisson1","poisson2","poisson3","gaussian1","gaussian2","gaussian3","ordinal1","ordinal2","ordinal3")
descriptor <- matrix(nr=1,c(rep("Categorical_pjk",3),rep("Poisson_k",3),rep("Gaussian_sjk",3),rep("Ordinal",3)))
...
output_learn <- fromJSON("/home/etienne/cylande/mixtcomp/JsonMixtcomp/utest/data/arg_list_test_learn_output.json")
param_mixtcomp_json <- output_learn$variable$param
arg_list_json <- toJSON(list(resGetData_lm = resGetData$lm,mcStrategy=mcStrategy,nbClass = 5,confidenceLevel = 0.95,mode="predict",paramList= param_mixtcomp_json),auto_unbox = T)
