...
class TestUtils(unittest.TestCase):
    ...
    def test_impute_model_DataFrame(self):
        model = impute_model(pd.DataFrame({"gauss": [1.5, 2.5], "pois": [1, 2], "mult": ["a", "b"], "z_class": [1, 2]}))

        self.assertDictEqual(model, {"gauss": "Gaussian", "pois": "Poisson", "mult": "Multinomial", "z_class": "LatentClass"})

    def test_impute_model_array(self):
        model = impute_model(np.array([[1, 2], [3, 4]]))
        self.assertDictEqual(model, {"var0": "Poisson", "var1": "Poisson"})

        model = impute_model(np.array([[1.0, 2.0], [3.0, 4.0]]))
    ...
    def test_impute_model_dict(self):
        model = impute_model({"gauss": [1.5, 2.5], "pois": [1, 2], "mult": ["a", "b"], "z_class": [1, 2]})
        self.assertDictEqual(model, {"gauss": "Gaussian", "pois": "Poisson", "mult": "Multinomial", "z_class": "LatentClass"})

    def test_format_data_basic_mode_without_dict(self):
        model = {
            "gauss": {"type": "Gaussian", "paramStr": ""},
            "pois": {"type": "Poisson", "paramStr": ""},
            "mult": {"type": "Multinomial", "paramStr": ""},
            "z_class": {"type": "LatentClass", "paramStr": ""},
        }

        data = {"gauss": [1.5, np.nan, None], "pois": [1, 4, 2], "mult": ["a", "b", None], "z_class": [1, 0, 1]}
        ...
        self.assertEqual(
            data, {"gauss": ["1.5", "?", "?"], "pois": ["1", "4", "2"], "mult": ["0", "1", "?"], "z_class": ["1", "0", "1"]}
        )
    ...
    def test_format_data_basic_mode_with_dict(self):
        ...
        self.assertEqual(
            data, {"gauss": ["1.5", "?", "?"], "pois": ["1", "4", "2"], "mult": ["1", "0", "?"], "z_class": ["1", "0", "1"]}
        )
    ...
    def test_format_output_basic_mode(self):
        with open("tests/mixtcomp_output.pickle", "rb") as f:
            ...
            self.assertListEqual(
                list(res["variable"]["param"]["mult"]["stat"].index),
                [
                    f"k: {k}, modality: {m}"
                    for k, m in zip(np.repeat(dictionary["z_class"]["old"], 4), dictionary["mult"]["old"] * 2)
                ],
            )
            self.assertListEqual(
                list(res["variable"]["param"]["mult2"]["stat"].index),
                [
                    f"k: {k}, modality: {m}"
                    for k, m in zip(np.repeat(dictionary["z_class"]["old"], 4), dictionary["mult2"]["old"] * 2)
                ],
            )
            self.assertListEqual(
                list(res["variable"]["param"]["gauss"]["stat"].index),
                [f"k: {k}, {m}" for k, m in zip(np.repeat(dictionary["z_class"]["old"], 2), ["mean", "sd"] * 2)],
            )
