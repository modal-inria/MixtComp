...
class TestUtilsGetter(unittest.TestCase):
    ...
    def test_get_param_numerical_gaussian(self):
        ...
        res = {"variable": {"param": {"gauss": param}, "type": {"gauss": "Gaussian"}}, "algo": {"nClass": 2}}
        out = get_param(res, "gauss")
        self.assertDictEqual(out.to_dict(), expected_out.to_dict())

    def test_get_param_numerical_latent_class(self):
        param = {"stat": pd.DataFrame([[1, 2, 3], [4, 5, 6]], columns=["median", "q0", "q1"], index=["k: 0", "k: 1"])}

        out = _get_param_numerical(param, 2, ["pi"])
        expected_out = pd.DataFrame([[1], [4]], columns=["pi"], index=["0", "1"])
        self.assertDictEqual(out.to_dict(), expected_out.to_dict())

        res = {"variable": {"param": {"z_class": param}, "type": {"z_class": "LatentClass"}}, "algo": {"nClass": 2}}
    ...
    def test_get_param_numerical_poisson(self):
        param = {
            "stat": pd.DataFrame(
                [[1, 2, 3], [4, 5, 6]], columns=["median", "q0", "q1"], index=["k: 0, lambda", "k: 1, lambda"]
            )
        }
        ...
        res = {"variable": {"param": {"pois": param}, "type": {"pois": "Poisson"}}, "algo": {"nClass": 2}}
    ...
    def test_get_param_multinomial(self):
        ...
        res = {"variable": {"param": {"mult": param}, "type": {"mult": "Multinomial"}}, "algo": {"nClass": 2}}
    ...
    def test_get_param_rank(self):
        param = {
            "pi": {
                "stat": pd.DataFrame([[1, 2, 3], [4, 5, 6]], columns=["median", "q0", "q1"], index=["k: 0, pi", "k: 1, pi"])
            },
            "mu": {"stat": {"k: 0": {"rank": np.array([3, 2, 1, 0])}, "k: 1": {"rank": np.array([0, 1, 2, 3])}}},
            "paramStr": "nModality: 4",
        }

        out = _get_param_Rank_ISR(param, 2)
        expected_out = {
            "pi": pd.DataFrame([[1], [4]], columns=["pi"], index=["0", "1"]),
        ...
        res = {"variable": {"param": {"rank": param}, "type": {"rank": "Rank_ISR"}}, "algo": {"nClass": 2}}
    ...
    def test_get_param_func_CS(self):
        ...
        res = {"variable": {"param": {"func": param}, "type": {"func": "Func_CS"}}, "algo": {"nClass": 2}}
    ...
    def test_get_tik(self):
        ...
        self.assertListEqual(out.tolist(), self.res["variable"]["data"]["z_class"]["stat"].to_numpy().tolist())
        self.assertIsInstance(out, np.ndarray)

        out = get_tik(self.res, log=True, empiric=False)
        self.assertListEqual(out.tolist(), np.log(self.res["variable"]["data"]["z_class"]["stat"].to_numpy()).tolist())
        self.assertIsInstance(out, np.ndarray)
    ...
    def test_get_tik_empiric(self):
        ...
        self.assertListEqual(
            out_log.tolist(),
            (
                self.res["mixture"]["lnProbaGivenClass"]
                - logsumexp(self.res["mixture"]["lnProbaGivenClass"], axis=1)[:, np.newaxis]
            ).tolist(),
        )
