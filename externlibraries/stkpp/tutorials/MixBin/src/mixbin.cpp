...
//    CsvToArray<CArray<Binary, Arrays::general_> > ctrans(rw, 0.2);
//    if (!ctrans.run())
//    {
//      stk_cerr << _T("An error occur when transferring the data.\nWhat: ") << trans.error();
//      return -1;
//    }
//    CArray<Binary, Arrays::general_> cdata;
//    cdata.move(*ctrans.p_data());
    //    CArray<Binary, Arrays::vector_, UnknownSize, 1> cvar;
    //    BernoulliModel< CArray<Binary, Arrays::vector_, UnknownSize, 1> > ucModel;
    ...
    for (int j= data.firstIdxCols(); j <= data.lastIdxCols(); ++j)
    {
      ...
      //      ucModel.run();
      //      stk_cout << "ucModel.law().prob() = " << ucModel.law().prob() << "\n";
    }
    //    JointBernoulliModel<CArray<Binary, Arrays::general_> > cjb(cdata);
    //    cjb.run();
    // create a multivariate Bernoulli model with the data set
...
//      stk_cout << "cjb.law().prob(j) = " << cjb.law().prob(j) << "\n";
    }
  }
