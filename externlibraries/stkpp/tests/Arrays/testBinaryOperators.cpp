...
int Test( int M, int N, bool output)
{
    ...
    CArray<TYPE, Arrays::general_, 3, 4, Arrays::by_col_> C1(M, N);
    CArray<bool, Arrays::general_, 3, 4, Arrays::by_row_> C2(M, N, 1);
    numbering(C1);
    stk_cout << "C1<TYPE,...> = \n" << C1;
    stk_cout << "C2<bool,...> = \n" << C2;
    // first test
    ...
    CArray<TYPE, Arrays::square_, 4, 4> C3(M, 0);
    ...
    CArray<TYPE, Arrays::general_, 4, 4> C5(M, N, 0);
    C5 = 3*C3 - TYPE(2)*C4;
