...
struct MultCoefImpl
{
  ...
  /** dot product */
  static void dot( ITContainer<Lhs> const& lhs, ITContainer<Rhs> const& rhs, ITContainer<Result>& res
                 , int iRow, int jCol)
  {
    res(iRow, jCol) = Type(0);
    Range const dotRange = Range::inf(lhs.rangeOfRow(iRow), rhs.rangeOfCol(jCol));
    for (int k=dotRange.firstIdx(); k<= dotRange.lastIdx(); ++k)
      res(iRow, jCol) += lhs(iRow, k) * rhs(k, jCol);
  }
  /** multiplication with one sized vectors */
  static void mult1( ITContainer<Lhs> const& lhs, ITContainer<Rhs> const& rhs, ITContainer<Result>& res
                   , int lhsCol, int rhsRow)
  {
    for (int i=lhs.firstIdxRows(); i<= lhs.lastIdxRows(); ++i)
      for (int j=rhs.firstIdxCols(); j<= rhs.lastIdxCols(); ++j)
        res(i,j) += lhs(i, lhsCol) * rhs(rhsRow, j);
  }
  /** multiplication with two sized vectors */
  static void mult2( ITContainer<Lhs> const& lhs, ITContainer<Rhs> const& rhs, ITContainer<Result>& res
                   , int lhsCol, int rhsRow)
  {
    for (int i=lhs.firstIdxRows(); i<= lhs.lastIdxRows(); ++i)
      for (int j=rhs.firstIdxCols(); j<= rhs.lastIdxCols(); ++j)
        res(i,j) += lhs(i, lhsCol  ) * rhs(rhsRow,   j)
                  + lhs(i, lhsCol+1) * rhs(rhsRow+1, j);
  }
  /** multiplication with three sized vectors */
  static void mult3( ITContainer<Lhs> const& lhs, ITContainer<Rhs> const& rhs, ITContainer<Result>& res
                   , int lhsCol, int rhsRow)
  {
    for (int i=lhs.firstIdxRows(); i<= lhs.lastIdxRows(); ++i)
      for (int j=rhs.firstIdxCols(); j<= rhs.lastIdxCols(); ++j)
        res(i,j) += lhs(i, lhsCol  ) * rhs(rhsRow,   j)
                  + lhs(i, lhsCol+1) * rhs(rhsRow+1, j)
                  + lhs(i, lhsCol+2) * rhs(rhsRow+2, j);
  }
  /** multiplication with one sized vectors */
  static void multVec1( ITContainer<Lhs> const& lhs, ITContainer<Rhs> const& rhs, ITContainer<Result>& res
                      , int lhsCol, int rhsRow)
  {
    for (int i=lhs.firstIdxRows(); i<= lhs.lastIdxRows(); ++i)
      res[i] += lhs(i, lhsCol) * rhs[rhsRow];
  }
  /** multiplication with two sized vectors */
  static void multVec2( ITContainer<Lhs> const& lhs, ITContainer<Rhs> const& rhs, ITContainer<Result>& res
                   , int lhsCol, int rhsRow)
  {
    for (int i=lhs.firstIdxRows(); i<= lhs.lastIdxRows(); ++i)
      res[i] += lhs(i, lhsCol) * rhs[rhsRow]
              + lhs(i, lhsCol+1) * rhs[rhsRow+1];
  }
  /** multiplication with three sized vectors */
  static void multVec3( ITContainer<Lhs> const& lhs, ITContainer<Rhs> const& rhs, ITContainer<Result>& res
                   , int lhsCol, int rhsRow)
  {
    for (int i=lhs.firstIdxRows(); i<= lhs.lastIdxRows(); ++i)
      res[i] += lhs(i, lhsCol) * rhs[rhsRow]
              + lhs(i, lhsCol+1) * rhs[rhsRow+1]
              + lhs(i, lhsCol+2) * rhs[rhsRow+2];
  }
  /** multiplication with one sized vectors */
  static void multPoint1( ITContainer<Lhs> const& lhs, ITContainer<Rhs> const& rhs, ITContainer<Result>& res
                        , int lhsCol, int rhsRow)
  {
    for (int j=rhs.firstIdxCols(); j<= rhs.lastIdxCols(); ++j)
      res[j] += lhs[lhsCol] * rhs(rhsRow, j);
  }
  /** multiplication with two sized vectors */
  static void multPoint2( ITContainer<Lhs> const& lhs, ITContainer<Rhs> const& rhs, ITContainer<Result>& res
                        , int lhsCol, int rhsRow)
  {
    for (int j=rhs.firstIdxCols(); j<= rhs.lastIdxCols(); ++j)
      res[j] += lhs[lhsCol] * rhs(rhsRow, j)
              + lhs[lhsCol+1] * rhs(rhsRow+1, j);
  }
  /** multiplication with three sized vectors */
  static void multPoint3( ITContainer<Lhs> const& lhs, ITContainer<Rhs> const& rhs, ITContainer<Result>& res
                   , int lhsCol, int rhsRow)
  {
    for (int j=rhs.firstIdxCols(); j<= rhs.lastIdxCols(); ++j)
      res[j] += lhs[lhsCol] * rhs(rhsRow, j)
              + lhs[lhsCol+1] * rhs(rhsRow+1, j)
              + lhs[lhsCol+2] * rhs(rhsRow+2, j);
  }
};
...
struct bp
{
   ...
   **/
  static void run(ITContainer<Lhs> const& lhs, ITContainer<Rhs> const& rhs, ITContainer<Result>& res)
  {
    // compute dimensions
  ...
  /** Default block */
  static void arrayToBlock( ITContainer<Lhs> const& lhs, Type* p_block
                          , int iRow, int iCol)
  {
  ...
  /** Default dimension */
  static void arrayToPanel( ITContainer<Rhs> const& rhs, Type* p_panel
                          , int iRow, int iCol)
  {
  ...
  static void blockByPanel( Type const* p_block, Type const* p_panel, Type* p_result
                          , ITContainer<Result>& res, int iRow, int iCol)
  {
  ...
  /** with panel size given */
  static void blockByPanel( Type const* p_block, Type const* p_panel, Type* p_result
                          , ITContainer<Result>& res, int iRow, int iCol, int pSize)
  {
  ...
  /** with panel size given */
  static void blockByPanel( Type const* p_block, Type const* p_panel, Type* p_result
                          , ITContainer<Result>& res, int iRow, int iCol, int pSize, int bSize)
  {
...
struct pb
{
  ...
  /** default dimensions */
  static void arrayToBlock( ITContainer<Rhs> const& rhs, Type* p_block
                          , int iRow, int iCol)
  {
  ...
  /** default dimensions */
  static void arrayToPanel( ITContainer<Lhs> const& lhs, Type* p_panel
                          , int iRow, int iCol)
  {
