...
/** @ingroup Arrays
  ...
  *
  * All memory is allocated there using the @c rangeOfCol method.
  *
...
class IArray2D : public IArray2DBase< typename hidden::Traits<Derived>::Type*, Derived>
{
  ...
  public:
    ...
    { return SubCol( this->allocator_, Range::inf(I, this->asDerived().rangeOfCol(j)), j);}
    /** access to a part of a column.
     *  @param j index of the column
     *  @return a reference in the range I of the column j of this
     **/
    inline Col col( int j) const
    { return Col( this->allocator_, this->asDerived().rangeOfCol(j), j);}
    ...
    { return SubRow(this->allocator_, Range::inf(J, this->asDerived().rangeOfRow(i)), i);}
    /** access to a part of a row.
     *  @param i index of the row
     *  @return a reference of the row i.
     **/
    inline Row row( int i) const
    { return Row(this->allocator_, this->asDerived().rangeOfRow(i), i);}
    ...
    void shiftRows( int rbeg)
    {
     ...
     int rinc = rbeg - this->firstIdxRows();
      ...
      if (rinc != 0)
      {
        ...
        if (this->isRef())
        { STKRUNTIME_ERROR_1ARG(IArray2D::shiftRows,rbeg,cannot operate on reference);}
        // translate rows_()
        IContainer2D::shiftRows(rbeg);
    ...
    void pushBackRows( int n=1)
    {
      ...
      else
      {
        ...
        for (int j=this->firstIdxCols(); j<=this->lastIdxCols(); j++)
        {
          ...
          Range range(this->asDerived().rangeOfCol(j));
    ...
    **/
    ...
    void update(int col)
    {
      ...
      if (this->asDerived().rangeOfCol(col) != this->rangeCol(col))
      { resizeCol(col, this->asDerived().rangeOfCol(col));}
    }
    ...
    void initializeCols(Range const& J)
    {
      ...
      for (int j=J.firstIdx(); j<=J.lastIdx(); j++)
      {
        try
        {
          ...
          this->initializeCol(j, this->rangeOfCol(j));
        }
