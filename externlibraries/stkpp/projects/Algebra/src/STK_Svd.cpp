...
Real Svd::bidiag(const Matrix& M, Point& D, Vector& F)
{
      ...
      )
  {
    ...
    Vector X( M.allocator(), rowRange0, iter);
    // Left Householder
    D[iter] = house(X);
    ...
    if ((iter < last_iter)||(M.sizeCols()>M.sizeRows()))
    {
      ...
      Point P(M.allocator(), colRange1, iter);
      // Right Householder
      F[iter] = house(P);
...
{ // Construction of V_
  ...
  {
    Vector W(V_.allocator(), V_.cols(), iter);
    W       = 0.0;
    W[iter] = 1.0;
  }
      ...
      )
  {
    ...
    if (beta)
    {
      ...
      Point  Vrow1(V_.allocator(), range1, iter1);
      // diagonal element
      Vrow1[iter1] = 1.0+beta;
      // ref on the column iter1
      Vector Vcol1(V_.allocator(), range2, iter1);
      // get the Householder vector
      Vcol1 = Point(U_.allocator(), range2, iter0);
      // Apply housholder to next cols
      for (int j=iter2; j<=ncolV_; j++)
      {
        Real aux;
        // ref on the column j
        Vector Vcolj( V_.allocator(), range2, j);
        // update column j
...
void Svd::compU()
{
  ...
  for (int iter=niter, iter1=niter+1; iter>=1; iter--, iter1--)
  {
    ...
    Vector X(U_.allocator(), Range(iter1,nrowU_, 0), iter);
    // ref of the row iter
    Point P(U_.allocator(), Range(iter,ncolU_, 0), iter);
    // Get beta and test
    Real beta = P[iter];
    if (beta)
    {
      ...
      { // product of U_iter by U_j
        Real aux;
        Vector Y(U_.allocator(), Range(iter1, nrowU_, 0), j); // ref on the column j
        // U_j = aux = beta * X'Y
