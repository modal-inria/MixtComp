...
void Qr::qr()
{
  ...
  { 
    ...
    Point  Rrow0(R_.allocator(), Range(iter, ncolr_, 0), iter);
    // A ref on the row of the matrix R_
    Point  Qrow0(Q_.allocator(), Range(iter, ncolq_, 0), iter);
    // A ref on the column iter of the matrix Q_ : will contain the
    // Householder vector
    Vector u(Q_.allocator(), Range(iter, nrowq_, 0), iter);
    ...
    if (beta)
    {
      ...
      for (int j=iter1; j<=ncolr_; j++)
      {
        ...
        Vector Z(Q_.allocator(), Range(iter1, nrowq_, 0), j);
...
void Qr::compQ()
{
  ...
  for (int iter=ncol, iter1=ncol+1; iter>=1; iter--, iter1--)
  {
    ...
    {
      // ref of the row iter
      Point P(Q_.allocator(), Range(iter,ncolq_, 0), iter);
      // ref of the column iter
      Vector X(Q_.allocator(), Range(iter1,nrowq_, 0), iter);
      // Update the cols from iter+1 to ncol
      for (int j=iter1; j<=ncolq_; j++)
      { Real aux;
        Vector Y(Q_.allocator(), Range(iter1,nrowq_, 0), j); // ref on the column j
        // Q_(iter, j) = beta * X'Y
...
void Qr::pushBackCol(Vector const& T)
{
  ...
  // update R_
  R_.col(ncolr_) = Rncolr.sub(R_.rangeOfCol(ncolr_));
}
...
void Qr::insertCol(Vector const& T, int const& pos)
{
  ...
  // update R_
  R_.col(pos) = Rpos.sub(R_.rangeOfCol(pos));
  R_.update(pos);
}
