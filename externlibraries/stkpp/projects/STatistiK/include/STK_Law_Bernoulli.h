...
class Bernoulli: public IUnivLaw<Binary>
{
  public:
    ...
    {
      if (prob<0) STKRUNTIME_ERROR_1ARG(Bernoulli::setProb,prob,prob must be >= 0);
      if (prob>1) STKRUNTIME_ERROR_1ARG(Bernoulli::setProb,prob,prob must be <= 1);
      prob_ = prob;
    }
     ...
     *  @param prob value of the probability giving the quantile
     **/
