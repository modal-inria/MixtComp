...
void heapSort( ITContainer<TContainer1D>& T)
{
  typedef typename hidden::Traits<TContainer1D>::Type Type;
  // number of elements
 ...
 **/
template< class TContainer1D>
void heapSort( ITContainer<TContainer1D> const& T, ITContainer<TContainer1D>& Tsort)
{
  typedef typename hidden::Traits<TContainer1D>::Type Type;
  // copy T in Tsort
  Tsort.asDerived() = T.asDerived();
  // number of elements
 ...
 *  @param T the container to sort
 **/
template< class TContainer1D, class TContainer1DInt>
void heapSort( ITContainer< TContainer1DInt> & I
             , ITContainer<TContainer1D> const& T
             )
{
  typedef typename hidden::Traits<TContainer1D>::Type Type;
 ...
 **/
template<class TContainer1D, class TContainer1DInt>
void applySort1D( ITContainer<TContainer1D>& T
                , ITContainer<TContainer1DInt> const& I
              )
{
  typedef typename hidden::Traits<TContainer1D>::Type Type;
  ...
  const int first = I.firstIdx(), last = I.lastIdx();
  for (int i=first; i<= last; i++)
  {
    A[i] = T[I[i]];
  }
 ...
 *  @param T the container to sort
 **/
template < class TContainer2D, class TContainer1DInt>
void applySort2D( TContainer2D& T
                , ITContainer< TContainer1DInt> const& I
                )
{
  typedef typename hidden::Traits<TContainer2D>::Type Type;
