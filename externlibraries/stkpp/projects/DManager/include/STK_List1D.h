...
struct Traits< List1D<_TYPE> >
{
  typedef _TYPE          Type;
  typedef List1D<_TYPE> Row;
  typedef List1D<_TYPE> Col;
  typedef List1D<_TYPE> SubRow;
  typedef List1D<_TYPE> SubCol;
  typedef List1D<_TYPE> SubArray;
  typedef List1D<_TYPE> SubVector;
  enum
  {
...
/** @ingroup DManager
  ...
  **/
template<class Type>
class List1D : public ITContainer1D< List1D<Type> >
             , public IContainerRef
{
  public:
    ...
    List1D() : Base(), IContainerRef(false)
    { initialize(Range()); }
  ...
  public:
    ...
    Cell const* const p_firstIdx() const { return p_first_;}
     ...
     **/
    inline List1D sub(Range const& J) const
    {
      if ((J.firstIdx()<this->firstIdx()))
      { STKOUT_OF_RANGE_1ARG(List1D::elt,J,J.firstIdx()<firstIdx());}
      if ((J.lastIdx()>this->lastIdx()))
      { STKOUT_OF_RANGE_1ARG(List1D::elt,J,J.lastIdx()>lastIdx());}
    ...
    void merge( List1D const& other)
    {
      ...
      // merge
      otherRef.p_first_->setLeft(p_last_);
      otherRef.p_last_->setRight(p_first_);
      p_last_->setRight(otherRef.p_first_);
      p_last_ = otherRef.p_last_;
      p_first_->setLeft(p_last_);
      ...
      otherRef.p_current_ = otherRef.p_first_;
    }
    ...
    **/
    ...
    List1D& operator=(const List1D &T)
    {
      ...
      { Cell *p1 = p_first_, *pt1 = T.p_first_;
      ...
      **/
     inline List1D<Type>& resize(Range const& I)
     {
       // check if there is something to do
       if ( this->range() == I) return *this;
       if (this->isRef())
       { STKRUNTIME_ERROR_1ARG(List1D::resize,I,cannot operate on references);}
       // translate beg
       shift(I.firstIdx());
  ...
  protected:
    Cell *p_first_;       ///< First Element of the List
    ...
    void initialize(Range const& I)
    {
      ...
      p_current_ = p_first_;            // CurrentPositionent cell is first cell
    }
    /** Protected function for deallocation.*/
    void freeMem()
    {
      if (this->isRef()) return;   // Nothing to do for ref
      Cell *p2, *p1 =p_first_;   // Auxiliary pointers for cells
      // for all cells
      for (int j=this->firstIdx(); j<=this->lastIdx(); j++)
    ...
    void moveCurrentPosition(int pos) const
    {
      ...
      else
      {
        if ((currentPosition_-pos) <= (pos-this->firstIdx()))
          ...
          for( currentPosition_ = this->firstIdx(), p_current_ = p_first_; currentPosition_!=pos; )
