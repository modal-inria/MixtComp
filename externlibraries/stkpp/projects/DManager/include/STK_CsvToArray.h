...
template<class Type>
struct CsvToArrayImpl<Type, CArray<Type, Arrays::general_> >
{
  static void run(TReadWriteCsv<Type> const& rw, CArray<Type, Arrays::general_>* p_data, Real const propMiss)
  {
    int jSize = 0;
    for(int jVar = rw.firstIdx(); jVar<=rw.lastIdx(); jVar++)
    {
