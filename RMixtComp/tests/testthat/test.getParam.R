...
test_that("getParamNumerical formats well", {
  ...
  param <- list(stat = matrix(1:9, nrow = 3))
  ...
  expect_equal(rownames(paramFormatted), c("k:1", "k:2", "k:3"))
  
  
  param <- list(stat = matrix(1:12, nrow = 4))
...
test_that("getParamMultinomial works well", {
  ...
  param <- list(stat = matrix(1:8, nrow = 8, ncol = 3), paramStr = "nModality: 4")
  ...
  expect_equal(colnames(paramFormatted), c("modality 1", "modality 2", "modality 3", "modality 4"))
  expect_equal(rownames(paramFormatted), c("k:1", "k:2"))
})
...
test_that("getParamRank_ISR works well", {
  ...
  param <- list(mu = list(stat = list("k: 0" = list(rank = matrix(4:1, nrow = 1), proba = 0.8),
                ...
                pi = list(stat = matrix(1:9, nrow = 3, ncol = 3)), 
...
test_that("getParamFunc_CS works well", {
  ...
  param <- list(alpha = list(stat = matrix(12:1, nrow = 12, ncol = 3)), # nbClass * nSub * 2
                beta = list(stat = matrix(1:12, nrow = 12, ncol = 3)),  # nbClass * nSub * nCoeff
                sd = list(stat = matrix(1:6, nrow = 6, ncol = 3)), # nbClass * nSub
