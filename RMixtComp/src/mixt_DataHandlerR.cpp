...
DataHandlerR::DataHandlerR(Rcpp::List rList) :
		nbInd_(0), nbVar_(0), rList_(rList) {
}

void DataHandlerR::writeInfo() const {
	for (std::map<std::string, std::string>::const_iterator it = info_.begin(), itEnd = info_.end(); it != itEnd; ++it) {
...
std::string DataHandlerR::listData() {
	...
	for (Index i = 0; i < rList_.size(); ++i) {
		...
		if (nbInd_ > 0 && nbInd_ != data.size()) {
			...
			sstm << "Variable: " << id << " has " << data.size() << " individuals, while the previous variable had " << nbInd_
					<< " individuals. All variables must have the same number of individuals." << std::endl;
			warnLog += sstm.str();
		}
...
std::string DataHandlerR::getData(std::string const& idData, Vector<std::string>& dataStr, Index& nbInd, std::string& paramStr) const {
	std::string warnLog;
