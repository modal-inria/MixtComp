...
public:
	...
	std::string getData(std::string const& idData, AugmentedData<DataType>& augData, Index& nbInd, std::string& paramStr, typename AugmentedData<DataType>::Type offset) const;
	...
	std::string getData(std::string const& idData, Vector<std::string>& dataStr, Index& nbInd, std::string& paramStr) const;
...
template<typename DataType>
std::string DataHandlerR::getData(std::string const& idData, AugmentedData<DataType>& augData, Index& nbInd, std::string& paramStr, typename AugmentedData<DataType>::Type offset) const {
	std::string warnLog;
	...
	if (dataMap_.find(idData) != dataMap_.end()) { // check if the data requested is present in the input data
		...
		for (Index i = 0; i < nbInd_; ++i) {
			...
			if (isValid) {
				...
				sstm << "In " << idData << ", individual i: " << i << " present an error. " << currStr << " is not recognized as a valid format." << std::endl;
				warnLog += sstm.str();
			}
		}
	} else {
		std::stringstream sstm;
		sstm << "Data from the variable: " << idData << " has been requested but is absent from the provided data. " << "Please check that all the necessary data is provided." << std::endl;
		warnLog += sstm.str();
	}
	return warnLog;
}
