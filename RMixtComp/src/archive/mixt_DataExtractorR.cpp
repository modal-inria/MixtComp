...
void DataExtractorR::exportVals(int indexMixture, bool hasModalities, std::string idName, const AugmentedData<Vector<int> >& augData,
	...
	for (int i = 0; i < augData.data_.rows(); ++i) {
		...
		if (augData.misData_(i).first != present_) {
			...
			for (std::vector<std::pair<int, Real> >::const_iterator itVec = dataStatStorage(i).begin(); itVec != dataStatStorage(i).end(); ++itVec) {
	...
	data_[indexMixture] = Rcpp::List::create(Rcpp::Named("completed") = dataR, Rcpp::Named("stat") = missingData);
}

/** Export function for classes (called from the composer) */
void DataExtractorR::exportVals(int indexMixture, std::string idName, const AugmentedData<Vector<Index> >& augData, const Matrix<Real>& tikC) {
	Rcpp::IntegerVector dataR(tikC.rows()); // vector to store the completed data set
	Rcpp::NumericMatrix tikR(tikC.rows(), tikC.cols()); // the empirical tik are completely exported, instead of the predominant modalities as in other categorical variables
	...
	mixtureName_[indexMixture] = idName;
	data_[indexMixture] = Rcpp::List::create(Rcpp::Named("completed") = dataR, Rcpp::Named("stat") = tikR);
}

/** Export function for gaussian model */
void DataExtractorR::exportVals(int indexMixture, bool hasModalities, std::string idName, const AugmentedData<Vector<Real> >& augData, const Vector<RowVector<Real> >& dataStatStorage) {
...
void DataExtractorR::exportVals(int indexMixture, bool hasModalities, std::string idName, const AugmentedData<Vector<int> >& augData, const Vector<RowVector<int> >& dataStatStorage) {
	Index offset = hasModalities ? minModality : 0;
	Rcpp::IntegerVector dataR(augData.data_.rows()); // vector to store the completed data set
	Rcpp::List missingData; // list to store all the missing values in a linear format
...
void DataExtractorR::exportVals(int indexMixture, std::string idName, const Vector<RankIndividual>& data, const std::vector<RankStat>& dataStat) {
	int nbInd = data.rows();
	...
	for (int i = 0, ie = nbInd; i < ie; ++i) {
		...
		if (!data(i).allPresent()) {
			...
			for (std::list<std::pair<RankVal, Real> >::const_iterator it = statStorageMu.begin(), ite = statStorageMu.end(); it != ite; ++it) {
				...
				individualProba.push_back(Rcpp::List::create(rankR, it->second));
			}
	...
	data_[indexMixture] = Rcpp::List::create(Rcpp::Named("completed") = lsData, Rcpp::Named("stat") = lsStat);
}

/** Export function for Functional model*/
void DataExtractorR::exportVals(int indexMixture, std::string idName, const Vector<Function>& data) {
	int nbInd = data.rows();
	...
	data_[indexMixture] = Rcpp::List::create(Rcpp::Named("data") = lsData, Rcpp::Named("time") = lsStat);
}
