 ...
 * EigenMatrixBaseAddons.h and the Iterator class is therefore not a member of
 * the namespace mixt::. It is accessible in MixtComp through the derived class
 * mixt::Matrix.
 */
...
public:
  ...
  Iterator(const Iterator &other)
      : i_(other.i_), j_(other.j_), rows_(other.rows_), p_mat_(other.p_mat_) {}
  ...
  bool operator<(const Iterator &it) {
    if (j_ < it.j_) {
      ...
      return false;
    } else if (i_ < it.i_) // in this case, j_ == it.j_, hence comparison must
                           // be done on i_
    {
      return true;
    }
