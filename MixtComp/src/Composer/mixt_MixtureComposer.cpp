...
std::string MixtureComposer::eStepObserved()
{
  ...
  for (int i = 0; i < nbSample_; ++i)
  {
    for (int k = 0; k < nbCluster_; k++)
    {
      ...
      for(MixtIterator it = v_mixtures_.begin(); it != v_mixtures_.end(); ++it)
      {
        if ((*it)->possibleNullProbability() == true) // observed likelihood is computed only when necessary
        {
          lnComp(k) += (*it)->lnObservedLikelihood(i, k);
        }
      }
    }
    ...
    if (tik_.row(i).sum() == std::log(0)) // the individual can not be generated by the model at all
    {
      std::stringstream sstm;
