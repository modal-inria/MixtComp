...
\begin{abstract}
This document resume the different "diagonal" mixture models that should be implemented
...
\item {\bf E step:} The current conditional probabilities that $z_{ik}=1$ for $i=1,\ldots,n$
  ...
  \begin{equation}
    \label{eq:condi}
    t^m_{ik}=t^m_k(\bx_i|\theta^{m-1})=\frac{ p^{m-1}_kh (\bx_i|{\blambda^{m-1}_k},\balpha )}
    {\sum_{l=1}^K  p^{m-1}_l h(\bx_i|\blambda^{m-1}_l,\balpha)}.
  \end{equation}
...
mixture components are overlapping or are in disparate proportions (see \cite{McLachlanPeel00}
, Section 2.21).
...
are the scale parameters. Assumptions on the position and shape parameters among the variables and the components
lead to define four families of mixture model. Let us write a multidimensional Gaussian mixture model in the from \verb+f_s*+
with \verb+b*+, the different ways to parametrize the scale parameters of a Gaussian mixture:
\begin{itemize}
\item \verb+sjk+ means that we have one scale parameter for each variable and for each component,
\item \verb+sk+ means that the scale parameters are the same for all the variables inside a component,
\item \verb+sj+ means that the scale parameters are different for each variable but are equals between the components,
\item and finally \verb+s+ means that the scale parameters are all equals.
\end{itemize}
...
\section{Multivariate  of The Beta Mixture Models}
